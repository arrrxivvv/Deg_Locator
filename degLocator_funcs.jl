...
function eigenZheevrInThr!( H, eLst, vLst, degObj; jobz = 'V' )
	eigenZheevr!( H, eLst, vLst, degObj.lwork, degObj.lrwork, degObj.liwork; work=view(degObj.workLst,:,Threads.threadid()), rwork=view(degObj.rworkLst,:,Threads.threadid()), iwork=view(degObj.iworkLst,:,Threads.threadid()), jobz = jobz );
end
...
function locateNon0Div( degObj, divBlst, threshold, N, param_dim; clipBnd = 0, isGOE3d=false, Bdim = 1 )
	( (a,b)->(a .= real.(b)) ).( degObj.non0LstRe, divBlst );
	posArr = degObj.divBposArr;
	negArr = degObj.divBnegArr;
	
	param_divide = degObj.param_divide[Bdim];
	if clipBnd > 0
		sliceLst = []
		for dim = 1 : param_dim
			ln = size( divBlst, dim );
	...
	posArrSl = [zeros(N) for ii = 1 : Threads.nthreads()];
	Threads.@threads for pos in degObj.posLst
		posArrSl[Threads.threadid()] .= degObj.non0LstRe[pos];
		for n = 1 : N
			posArr[pos,n] =  posArrSl[Threads.threadid()][n] > threshold ;
			negArr[pos,n] =  posArrSl[Threads.threadid()][n] < negThres;
		end
	end
	
	if !isGOE3d 
		posCount = sum( degObj.divBposArr, dims = [1,2,3] );
		negCount = sum( degObj.divBnegArr, dims = [1,2,3] );
		posLocLst = Vector{ Array{ Int64,2 } }(undef,N);
		negLocLst = Vector{ Array{ Int64,2 } }(undef,N);
		for n = 1:N
			posLocLst[n] = cartIndLstToArr( findall( degObj.divBposArr[..,n] ), param_dim );
			negLocLst[n] = cartIndLstToArr( findall( degObj.divBnegArr[..,n] ), param_dim );
		end	
	else 
		dimLst = [1 : param_dim;];
		deleteat!(dimLst, Bdim);
		posCountArr = dropdims( sum( degObj.divBposArr; dims=dimLst ); dims=Tuple(dimLst) );
		negCountArr = dropdims( sum( degObj.divBnegArr; dims=dimLst ); dims=Tuple(dimLst) );
		posCount = [ posCountArr[il,:] for il = 1 : param_divide ];
		negCount = [ negCountArr[il,:] for il = 1 : param_divide ];
		# @infiltrate
		...
		divBindPol = [degObj.divBposArr, degObj.divBnegArr];
...
function locateNon0Div_alloc( degObj, divBlst, threshold, N, param_dim; clipBnd = 0, isGOE3d=false, Bdim = 1 )
	divBlstReal = (p-> real.(p)).(divBlst);
...
function locator_Eigen_work_init( degObj )
	# @infiltrate
...
function locator_Eigen_threaded!( degObj, HmatFun; isGC = false )
	...
	lwork, lrwork, liwork, workLst, rworkLst, iworkLst = degObj.lwork, degObj.lrwork, degObj.liwork, degObj.workLst, degObj.rworkLst, degObj.iworkLst;
	
	@info( "Eigen: " )
	tFull = @timed begin
	if degObj.enumSaveMem == memNone
		Threads.@threads for ii in degObj.posLst
			HmatFun( degObj.HmatLst[ii], degObj.param_mesh[ii] );
	...
	else #if degObj.enumSaveMem == memEig
		# @error( "isSaveMem" );
		meshSl = selectdim( degObj.param_mesh, degObj.param_dim, 1 );
		ElstSl = selectdim( degObj.Elst, degObj.param_dim, 1 );
		if degObj.enumSaveMem != rootFind && degObj.enumSaveMem != rootFindLanczos
			vecLstSl = selectdim( degObj.vecLst, degObj.param_dim, 1 );
		else
			vecLstDum = similar( degObj.HmatLst[1], 1, 0 );
		end
		for id = 1 : degObj.param_divide[degObj.param_dim]
			meshSl = selectdim( degObj.param_mesh, degObj.param_dim, id );
			ElstSl = selectdim( degObj.Elst, degObj.param_dim, id );
			if degObj.enumSaveMem != rootFind && degObj.enumSaveMem != rootFindLanczos
				vecLstSl = selectdim( degObj.vecLst, degObj.param_dim, id );
			end
			Threads.@threads for i2d in CartesianIndices(selectdim(degObj.posLst,degObj.param_dim,1))
				# ii = [i2d,id];
				HmatFun( degObj.HmatLst[i2d], meshSl[i2d] );
				eigenZheevr!( degObj.HmatLst[i2d], ElstSl[i2d], ( ( degObj.enumSaveMem == rootFind || degObj.enumSaveMem == rootFindLanczos ) ? vecLstDum : vecLstSl[i2d] ), lwork, lrwork, liwork; work=view(workLst,:,Threads.threadid()), rwork=view(rworkLst,:,Threads.threadid()), iwork=view(iworkLst,:,Threads.threadid()), jobz = ( ( degObj.enumSaveMem == rootFind || degObj.enumSaveMem == rootFindLanczos ) ? 'N' : 'V') );
			end
		end
	end	
	end # time
...
function locator_Eigen_link_layered!( degObj, HmatFun; yesLink = true )
	lwork, lrwork, liwork, workLst, rworkLst, iworkLst = degObj.lwork, degObj.lrwork, degObj.liwork, degObj.workLst, degObj.rworkLst, degObj.iworkLst;
	
	posLstSlice = degObj.posLstSlice;
	nDimSlc = degObj.param_dim - 1;
	
	shLst = zeros( Int64, nDimSlc );
	meshSl = selectdim( degObj.param_mesh, degObj.param_dim, 1 );
	# linkLstSl = selectdim( degObj.linkLst, degObj.param_dim, 1 );
	if degObj.enumSaveMem == rootFind || degObj.enumSaveMem == rootFindLanczos
		vecLstDum = similar( degObj.HmatLst[1],1,0 );
	end
	for id = 1 : degObj.param_divide[degObj.param_dim]
		meshSl = selectdim( degObj.param_mesh, degObj.param_dim, id );
		# @time begin
		Threads.@threads for i2d in posLstSlice
			HmatFun( degObj.HmatLst[i2d], meshSl[i2d] );
			eigenZheevr!( degObj.HmatLst[i2d], degObj.Elst[i2d], ( ( degObj.enumSaveMem == rootFind || degObj.enumSaveMem == rootFindLanczos ) ? vecLstDum : degObj.vecLst[i2d]), lwork, lrwork, liwork; work=view(workLst,:,Threads.threadid()), rwork=view(rworkLst,:,Threads.threadid()), iwork=view(iworkLst,:,Threads.threadid()) );
		end
		# end
		
		if degObj.enumSaveMem != rootFind && degObj.enumSaveMem != rootFindLanczos
			for dim = 1 : nDimSlc
				linkLstSl = selectdim( degObj.linkLst[dim], degObj.param_dim, id );
				vecLstSh = ShiftedArrays.circshift(degObj.vecLst, ntuple( i -> i==dim ? -1 : 0, nDimSlc ));
				# shLst .= 0;
				# shLst[dim] = 1;
				Threads.@threads for i2d in posLstSlice
					dotEachCol!( linkLstSl[i2d], vecLstSh[i2d], degObj.vecLst[i2d] );
				end
			end
			
			if id > 1
				dim = degObj.param_dim;
				linkLstSl = selectdim( degObj.linkLst[dim], degObj.param_dim, id-1 );
				Threads.@threads for i2d in posLstSlice
					dotEachCol!( linkLstSl[i2d], degObj.vecLst[i2d], degObj.vecLstPrev[i2d] );
				end
			end
			
			# @infiltrate
			# @time begin
			if id == degObj.param_divide[degObj.param_dim]
				dim = degObj.param_dim;
				linkLstSl = selectdim( degObj.linkLst[dim], degObj.param_dim, id );
				Threads.@threads for i2d in posLstSlice
					dotEachCol!( linkLstSl[i2d], degObj.vecLst1st[i2d], degObj.vecLst[i2d] );
				end
			end
			# end
			
			if id == 1
				((a,b)->(a.=b)).(degObj.vecLst1st, degObj.vecLst );
			end
			((a,b)->(a.=b)).(degObj.vecLstPrev, degObj.vecLst );
		end
	end
...
function locator_div(degObj, HmatFun; isGOE3d = false)
	...
	if degObj.enumSaveMem != memEigLink
	...
	elseif degObj.enumSaveMem == memEigLink
		@info( "Eigen and link" );
		tFull = @timed locator_Eigen_link_layered!( degObj, HmatFun  );
		@info( timeMemStr( tFull.time, tFull.bytes ) )
	end
	...
	if degObj.enumSaveMem != rootFind && degObj.enumSaveMem !== rootFindLanczos
		...
		@info "locateNon0"
		if !isGOE3d
			tFull = @timed posCount, negCount, posLocLst, negLocLst = locateNon0Div( degObj, non0Lst, threshold, degObj.N, degObj.param_dim; clipBnd = clipBnd );
			@info( timeMemStr( tFull.time, tFull.bytes ) )
			...
			for iB = 1 : length(degObj.BfieldLst)
				tFull = @timed posCount1, negCount1, posLocLst1, negLocLst1 = locateNon0Div( degObj, non0Lst[iB], threshold, degObj.N, degObj.param_dim; clipBnd = clipBnd, isGOE3d = isGOE3d, Bdim = BdimLst[iB] );
				@info( timeMemStr( tFull.time, tFull.bytes ) )
				# @infiltrate
		...
		if !isGOE3d
			@info( "rootFind" );
			if degObj.enumSaveMem == rootFind
				posCount, negCount, posLocLst, negLocLst = locate_rootFind_full( degObj, HmatFun );
			else 
				posCount, negCount, posLocLst, negLocLst = locate_rootFind_Lanczos( degObj, HmatFun );
			end
		end
	end
	
	return posCount, negCount, posLocLst, negLocLst, degObj.BfieldLst, degObj.non0LstRe;
	# divBlstReal;
end
