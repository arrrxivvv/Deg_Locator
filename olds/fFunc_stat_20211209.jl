...
struct FfObj
	...
	function FfObj( N, param_dim, param_divide, itNum, posLocLst, negLocLst, fObjOpt )
		...
		new( N, param_divide, fFuncLn, idPolEnd, itNum, locLst, fFuncLst, tmp, flip, flip2, avg, tmpMsc );
	end
end
		...
		for it = 1 : fObj.itNum
			...
			for idPol1 = 1:fObj.idPolEnd
				...
				for idPol2 = 1:fObj.idPolEnd
					...
					view( fObj.fFuncLst, n, idPosNeg, .. ) .+= fObj.avg[thrId()];
					# @time view( fObj.fFuncLst, n, idPosNeg, .. ) .= fObj.tmp[thrId()];
					idPosNeg += 1;
					# readline();
				end
			end
...
function fFunc_stat_from_file_base( fFunc_statFunc!, oFileNameMain, N, param_divide, num_it, seedFedStart, fileNameMod = "", param_dim = 3, fileOutNameMod = "",fObjOpt = "1d"; isDistilled = false )
	...
	fFuncSolver = FfObj( N, param_dim, param_divide, num_it,data["posLocLst"], data["negLocLst"], fObjOpt );
