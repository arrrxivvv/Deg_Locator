...
function locator_div(degObj, HmatFun)
	...
	# Elst, vecLst = locator_Eigen_threaded!( degObj, HmatFun );
...
function locator_div_GUE_ratio( degObj; ratio=[1,1,1], Hmat_seed = 1234, polarity = +1, maxDepth = 0, param_divide_next = [1,2,2] )
	if degObj.param_dim !=3
		@error "GUE should be dim 3"
	end
	
	H_GUElst = HlstFunc( H_GUE, degObj.param_dim, degObj.N );
