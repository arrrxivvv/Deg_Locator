...
function divB_profile( Nlst=[20], num_it_main=2, param_divide=50, seedFedstart = -1; scale=1, ratio=[1,1,1], fileNameMod = "" )
	param_dim = 3;
	symFact = 4;
	...
	for itN = 1 : length(Nlst)
		...
		for itMain = 0:num_it_main-1
			...
			it = itStart;
			posNlst[it,:], negNlst[it,:], posLocLst[it], negLocLst[it], H_GUE_lst[it] = locator_div_GUE_ratio( N, param_divide; ratio = ratio, Hmat_seed = seedFed ); 
			# display(ratio);
