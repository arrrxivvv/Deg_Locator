...
function locator_div(N, HmatFun, param_dim, param_divide, param_max, param_min; maxDepth = 0, depth = 0, nSpecific = 0, dim_hold = 0)
		...
		for iPol = 1 : 2
			...
			for n = 1 : Nlimited
				...
				for it = 1 : itStop
					...
					param_divide_next = Int64.( 3 * ones(param_dim) );
					if dim_hold != 0
...
function locator_div_GUE( N, param_divide_num; Hmat_seed = 1234, maxDepth = 0, polarity = +1)
	...
	H_GUElst = [ zeros( Complex{Float64}, N,N ) for ii = 1:2, jj = 1:param_dim ];
	if Hmat_seed > 0
		Random.seed!(Hmat_seed);
	end
	for it in eachindex(H_GUElst)
		H_GUElst[it] = H_GUE(N);
	end
...
function locator_div_GUE_ratio( N, param_divide_num; ratio=[1,1,1], Hmat_seed = 1234, polarity = +1, isOrtho = false )
	param_dim = 3;
	param_max = [2*pi,2*pi,2*pi];
	param_min = [0,0,0];
