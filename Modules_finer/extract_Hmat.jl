...
function readLocLst( N, param_divide, Mmat_seed, avgNum; fileNameMod = "" )
	param_dim = 3;
...
function getH1Lst( posLoc1Lst, negLoc1Lst, HmatFun1, param_dim, param_grids, param_step )
	paramLstPol = [posLoc1Lst, negLoc1Lst];
	N = size(posLoc1Lst)[1];
	HmatLstPol = [];
	# paramLstPol = [];
	for idPol = 1:2
		paramLst = paramLstPol[idPol];
		...
		# for n = 1 : N
			...
			# end
		# end
		HmatLst = [ [ HmatFun1( paramLst[n][iDeg,:]... ) for iDeg = 1 : size(paramLst[n])[1] ] for n = 1 : N ];
...
function nToParam( nLst, param_dim, param_grids, param_step )
	...
	for j = 1 : param_dim
		param_arr[j] = param_grids[j][nLst[j]] + param_step[j] ;
	end
...
function printHmatLstEigenTr( HmatLstPol, N, n )
	# for idPol = 1 : 2
		# for it = 1 : length( HmatLstPol[1][n] )
			# display( round.( HmatLstPol[1][n][it]; digits=2 ) );
		# end
	...
	for it = 1 : length( HmatLstPol[1][n] )
		ElstPos = eigen( HmatLstPol[1][n][it] ).values;
		ElstNeg = eigen( HmatLstPol[2][n][it] ).values;
		ElstPos = ElstPos .- sum(ElstPos) / N;
		ElstNeg = ElstNeg .- sum(ElstNeg) / N;
		display( hcat( round.( ElstPos; digits=2 ), zeros(N), round.( ElstNeg; digits=2 ) ) );
	end
end
