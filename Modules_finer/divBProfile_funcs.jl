...
function shLocLst( locLst, param_max, param_step, dim, shId )
	if shId == 1
		return locLst;
	end
	
	locLstCell = broadcast.( +, locLst, param_step/2 );
	if shId == 2
		for locs in locLstCell
			locs[:,dim] = mod.( param_max .- locs[:,dim], param_max );
		end
	elseif shId == 3
		for locs in locLstCell
			locs[:,dim] = mod.( param_max/2 .- locs[:,dim], param_max );
		end
	elseif shId == 4
		for locs in locLstCell
			locs[:,dim] = mod.( param_max/2 .+ locs[:,dim], param_max );
	...
	locLst = broadcast.( -, locLstCell, param_step/2  );
	return locLst;
end

function divB_profile( Nlst=[20], num_it_main=2, param_divide=50, seedFedstart = -1;scale=1, ratio = [1,1,1], maxDepth = 0, fileNameMod = "" )
	param_dim = 3;
	symFact = 4;
	stepFine = 2^maxDepth;
	...
	for itN = 1 : length(Nlst)
		...
		for itMain = 0:num_it_main-1
			...
			it = itStart;
			posNlst[it,:], negNlst[it,:], posLocLst[it], negLocLst[it], H_GUE_lst[it] = locator_div_GUE_ratio( N, param_divide; ratio = ratio, Hmat_seed = seedFed, maxDepth = maxDepth ); 
			...
			for itSym in indCartPosNeg
				...
				for dim = 1 : param_dim
					itSymDim = itSym[dim];
					posLocLstTmp = shLocLst( posLocLstTmp, param_max[dim], param_step[dim]*stepFine, dim, itSymDim );
					negLocLstTmp = shLocLst( negLocLstTmp, param_max[dim], param_step[dim]*stepFine, dim, itSymDim );
				end
		...
		fileNameAttr = string( fileNameAttrFunc( N, param_divide, num_it, seedFedstart ), "_", fileNameMod );
		fileNameMain = "deg";
		if scale != 1'
