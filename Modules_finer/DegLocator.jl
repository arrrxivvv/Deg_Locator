...
function BfieldFromVec( dim, vecLst )
...
function Bfield( point_origin, HmatFun, dr_incre, N, dim; n_level=0 )
	...
	if n_level != 0
		vecLst_n = Array{ typeOfNvec,dim }( undef, Tuple(incrDimLst) );
		for it = 1 : length(vecLst_n)
			# colN = vecLst[it][:,n_level];
			# vecLst_n[it] = reshape( colN, length(colN), 1 );
			vecLst_n[it] = vecLst[it][:,n_level:n_level];
		end
		return BfieldFromVec( dim, vecLst_n );
	else 
		return BfieldFromVec( dim, vecLst );
	end
	
	return BfieldLst;
end

function locator( N, num_ini, num_it )
	# num_ini = 30;
	dim = 3;
	...
	for it_try = 1:num_ini
		for n = 1:N
			...
			for it = 1:num_it
				...
				# Bvec = BvecLst[n,..];
				BvecLst = Bfield( point, HmatFun, dr_incre, N, dim; n_level = n );
				Bvec = BvecLst[1,..];
				Bvec = Bvec[[3,2,1]];
	...
	oFileName = string( "deg_", "N_", N, "_seed_", Hmat_seed, "_ini_seed_", ini_seed, "_iniPoints_", num_ini, "_it_", num_it );
	varFileSub = ".jld";
	varFileName = string( oFileName, varFileSub );
	save( varFileName, "N", N, "seed", Hmat_seed, "H_GUElst", H_GUElst, "iniPoints", num_ini, "iniLst", ini_arr,  "iterations", num_ini, "degLst", degen_arr );
	...
	printlnLog("trace iteration count: ", num_it);
	printlnLog("number of trial points: ", num_ini);
	printlnLog( "number of degeneracy points: " );
	for n = 1 : N
		printlnLog( "state ", n, " : ", length(degen_arr[n]) );
	end
	printlnLog( "total number of degeneracy points: ", sum( length.(degen_arr) ) );
	
	t_elapse = time() - t_start;
	printlnLog( "time elapsed: ", t_elapse );
end
