...
function locator_Eigen_threaded( N, HmatFun, param_dim, param_divide, param_grids, dummyMat )
	...
	@debug "Elst" Elst[1,2,1];
	@debug "vecLst" vecLst[1,2,1];
...
function locator_div(N, HmatFun, param_dim, param_divide, param_max, param_min)
	...
	Elst, vecLst = locator_Eigen_threaded( N, HmatFun, param_dim, param_divide, param_grids, dummyMat );
	...
	@debug "Bfield" BfieldLst[1][2,2,2];
	if param_dim == 2
		non0Lst = BfieldLst[1];
		divBlst = [];
	elseif param_dim >= 3
