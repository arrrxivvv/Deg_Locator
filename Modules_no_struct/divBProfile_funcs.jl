...
function divB_profile( param_dim = 2, Nlst=[20], num_it_main=2, param_divide=50, seedFedstart = -1; scale=1, ratio=nothing, fileNameMod = "", maxDepth = 0, param_divide_next = nothing )
	if ratio == nothing
	...
	for itN = 1 : length(Nlst)
		...
		for it = 1:num_it
			...
			# it = itStart;
			# print( "locator_ratio_time: " )
			posNlst[it,:], negNlst[it,:], posLocLst[it], negLocLst[it], H_GUE_lst[it] = locator_func_ratio( N, param_divide; ratio = ratio, Hmat_seed = seedFed, maxDepth = maxDepth, param_divide_next = param_divide_next ); 
